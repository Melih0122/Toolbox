...
echo   %R%[90m║%R%[32m 11.%C%[36m Origin               %R%[90m║%R%[32m  37.%C%[36m Any Video Converter    %R%[90m║%R%[32m 63.%C%[33m Blender                     %R%[90m║%R%[0m
...
(
echo ---------------------------------------------------------------------------------------------------------------------------
echo [%date% - %time%] ^| Katılımsız ^| OpenBat - Katılımsız dosyası giriş bölümü hazırlandı.
) >> %Location%\Logs
...
(
echo [%date% - %time%] ^| Katılımsız ^| OpenBat - Katılımsız dosyası son bölümü uygulandı. 
echo ---------------------------------------------------------------------------------------------------------------------------
) >> %Location%\Logs
Call :RegTopla
(
...
echo.
echo :wget1
echo Call :InternetControl
echo FOR /F "tokens=1" %%%%i in ^('FIND "%%~1" %%Location%%\Extra\Links.txt'^) do set link=%%%%i
echo %%Location%%\Files\wget.exe -c -q --no-check-certificate --show-progress "%%link%%" -O %%Location%%\Download\%%~1"
echo "%%Location%%\Download\%%~1" %%~2
goto :eof
...
Call :LogSave "wget2" "%~1 indirildi. %link%"
...
Call :InternetControl
FOR /F "tokens=1" %%i in ('FIND "%~1" %Location%\Extra\Links.txt') do set link=%%i
Call :LogSave "wget3" "%~1 indirildi. %link%"
...
Find "%~n1" %Mount%\OgnitorenKs.Toolbox\Katilimsiz\OgnitorenKs.Katilimsiz.bat > NUL 2>&1
	if %errorlevel%==0 (echo   %R%[91m HATA! %~n1 katılımsız dosyasına eklenmiş / Already attached%R%[0m
						timeout /t 2 /nobreak > NUL
						goto :eof)
echo [%date% - %time%] ^| Katılımsız ^| "%~n1" eklendi / added. >> %Location%\Logs
(
echo.
echo :: -----------------------------------------------------------------------------------
...
Find "05x86" %Mount%\OgnitorenKs.Toolbox\Katilimsiz\OgnitorenKs.Katilimsiz.bat > NUL 2>&1
	if %errorlevel%==0 (echo   %R%[91m HATA! All in One Runtimes katılımsız dosyasına eklenmiş / Already attached%R%[0m
						timeout /t 2 /nobreak > NUL
						goto :eof)
echo [%date% - %time%] ^| Katılımsız ^| All IN ONE RUNTIMES eklendi / added. >> %Location%\Logs
(
echo.
echo :: -----------------------------------------------------------------------------------
...
Find "ISLC" %Mount%\OgnitorenKs.Toolbox\Katilimsiz\OgnitorenKs.Katilimsiz.bat > NUL 2>&1
	if %errorlevel%==0 (echo   %R%[91m HATA! ISLC katılımsız dosyasına eklenmiş / Already attached%R%[0m
						timeout /t 2 /nobreak > NUL
						goto :eof)
echo [%date% - %time%] ^| Katılımsız ^| ISLC eklendi / added. >> %Location%\Logs
...
Find "AnyVideoConverter" %Mount%\OgnitorenKs.Toolbox\Katilimsiz\OgnitorenKs.Katilimsiz.bat > NUL 2>&1
	if %errorlevel%==0 (echo   %R%[91m HATA! Any Video Converter katılımsız dosyasına eklenmiş / Already attached%R%[0m
						timeout /t 2 /nobreak > NUL
						goto :eof)
echo [%date% - %time%] ^| Katılımsız ^| AnyVideoConverter eklendi / added. >> %Location%\Logs
(
echo.
echo :: -----------------------------------------------------------------------------------
...
Find "WiseCare365" %Mount%\OgnitorenKs.Toolbox\Katilimsiz\OgnitorenKs.Katilimsiz.bat > NUL 2>&1
	if %errorlevel%==0 (echo   %R%[91m HATA! Wise Care 365 katılımsız dosyasına eklenmiş / Already attached%R%[0m
						timeout /t 2 /nobreak > NUL
						goto :eof)
echo [%date% - %time%] ^| Katılımsız ^| WiseCare365 eklendi / added. >> %Location%\Logs
(
echo.
echo :: -----------------------------------------------------------------------------------
...
Find "TaskbarX" %Mount%\OgnitorenKs.Toolbox\Katilimsiz\OgnitorenKs.Katilimsiz.bat > NUL 2>&1
	if %errorlevel%==0 (echo   %R%[91m HATA! TaskbarX katılımsız dosyasına eklenmiş / Already attached%R%[0m
						timeout /t 2 /nobreak > NUL
						goto :eof)
echo [%date% - %time%] ^| Katılımsız ^| TaskbarX eklendi / added. >> %Location%\Logs
(
echo.
echo :: -----------------------------------------------------------------------------------
...
Find "OgnitorenKs Toolbox kuruluyor / installing..." %Mount%\OgnitorenKs.Toolbox\Katilimsiz\OgnitorenKs.Katilimsiz.bat > NUL 2>&1
	if %errorlevel%==0 (echo   %R%[91m HATA! OgnitorenKs.Toolbox katılımsız dosyasına eklenmiş / Already attached%R%[0m
						timeout /t 2 /nobreak > NUL
						goto :eof)
echo [%date% - %time%] ^| Katılımsız ^| OgnitorenKs Toolbox eklendi / added. >> %Location%\Logs
...
Find "Masaustu dosyalari yukleniyor" %Mount%\OgnitorenKs.Toolbox\Katilimsiz\OgnitorenKs.Katilimsiz.bat > NUL 2>&1
	if %errorlevel%==0 (echo   %R%[91m HATA! Masaustu dosyaları katılımsız dosyasına eklenmiş / Already attached%R%[0m
						timeout /t 2 /nobreak > NUL
						goto :eof)
echo [%date% - %time%] ^| Katılımsız ^| Masaüstüne dosyalar eklendi / added. >> %Location%\Logs
...
Find "Gorev Zamanlayici" %Mount%\OgnitorenKs.Toolbox\Katilimsiz\OgnitorenKs.Katilimsiz.bat > NUL 2>&1
	if %errorlevel%==0 (echo   %R%[91m HATA! Performans ayarları katılımsız dosyasına eklenmiş / Already attached%R%[0m
						timeout /t 2 /nobreak > NUL
						goto :eof)
echo [%date% - %time%] ^| Katılımsız ^| Görev Zamanlayıcı ayarları eklendi / added. >> %Location%\Logs
...
Powershell -command "Compress-Archive -LiteralPath %Mount%\OgnitorenKs.Toolbox -DestinationPath C:\Users\%username%\Desktop\Yedek.Katılımsız.zip -Force"
...
echo [%date% - %time%] ^| Katılımsız ^| "%Yedek%" yolundan katılımsız yedek imaja yüklendi. Mount="%Mount%" >> %Location%\Logs
