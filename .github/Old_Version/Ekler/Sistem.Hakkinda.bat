...
echo [%date% - %time%] ^| SistemHakkinda ^| Sistem hakknda b”lm "Sistem.Hakkinda.bat" dosyas zerinden ‡alŸtrld. >> %konum%\Logs


:Information
bcdedit > %Logs%\Bcdedit.txt
wmic os get RegisteredUser, CSName, Caption, OSArchitecture, BuildNumber /value > %Logs%\OS.txt
wmic ComputerSystem Get Manufacturer, Model, TotalPhysicalMemory /value > %Logs%\ComputerSystem.txt
...
:: Log ‡kt alma b”lm
(
echo  ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»
echo                                       S˜STEM HAKKINDA
echo  ÌÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹
FOR /F "tokens=2 delims='='" %%a in ('FIND "RegisteredUser" %Logs%\OS.txt') do (
	FOR /F "tokens=2 delims='='" %%b in ('FIND "CSName" %Logs%\OS.txt') do (
		echo   ^> Bilgisayar: %%b
		echo   ^> Kullanc : %%a    
...
echo  ÌÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹
echo   ^> Format Tarihi: %formatgun%.%formatay%.%formatyil%
echo  ÌÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹
echo   ^> Sistem:%caption% / %osarchitecture% / %ImageBuild% / %isderleme% / %uefi%
echo   ^> Anakart: %boardmarka% %board% / Soket: %boardsoket% / Bios: %bios%
echo   ^> CPU: %cpu%
echo   ^> CPU €ekirdek: %cpucekirdek% / Threads: %cpusanalcekirdek% / L2:%cpuL2% MB / L3: %cpuL3% MB / CPU MHZ: %cpuclock%
FOR /F "tokens=2 delims='='" %%a in ('FIND "Caption" %Logs%\Monitor.txt') do ( 
	FOR /F "tokens=2 delims='='" %%b in ('FIND "CurrentHorizontalResolution" %Logs%\GPU.txt') do (
		FOR /F "tokens=2 delims='='" %%c in ('FIND "CurrentVerticalResolution" %Logs%\GPU.txt') do (
			FOR /F "tokens=2 delims='='" %%d in ('FIND "CurrentRefreshRate" %Logs%\GPU.txt') do (
				echo   ^> Monit”r: %%a / %%d HZ / %%b x %%c 
				)
			)
		)
	)
echo   ^> RAM: %ramtotal2% GB / %ramspeed% MHZ / Soket: %RamType%
echo  ÌÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹
echo               Slot       Kapasite    Marka              Model        MHZ
FOR /F "skip=1 tokens=*" %%a IN ('WMIC memorychip Get Manufacturer^,BankLabel^,Capacity^,PartNumber^,Speed') DO (
	echo  ^> RAM: %%a)
echo            Kapasite       Driver              Model%
FOR /F "skip=1 tokens=*" %%a IN ('wmic path win32_videocontroller get name^,AdapterRAM^,DriverVersion') DO (
	echo   ^> GPU: %%a)
) >> %konum%\Logs
:: ==============================================================================================================================
