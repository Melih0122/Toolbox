...
echo   %ESC%[35m^|%ESC%%ESC%[92m 26.%ESC%%ESC%[33m WinRAR%ESC%[0m               ^| %ESC%[92m 52.%ESC%%ESC%[97m €oklu Se‡im Yap%ESC%[0m        %ESC%[35m^|%ESC%%ESC%[92m 76.%ESC%%ESC%[93m Ping ™l‡er%ESC%[0m                  %ESC%[35m^|%ESC%[0m 
...
powershell -command "Expand-Archive -Force '%down%\oal.zip' '%down%"
DEL /F /Q /A "%down%\oal.zip" 
...
echo  DirectX kuruluyor...
echo "C:\Gerekli\DirectX\DirectX.exe" /silent
echo Islem tamamlandi. Komut ekrani kapatiliyor.
echo reg delete "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v 4 /f 
echo exit
) > %down%\Kurulum.bat
Call :PowerRun
Powershell -command "Compress-Archive -Path %down% -DestinationPath %down%\Gerekli.zip"
%PowerRun% Powershell -command "Expand-Archive -Force '%down%\Gerekli.zip' '%MerkezWim%'"
Call :regyukle 
...
:wgetoff
Call :konum
set down=%konum%\Download\Gerekli
DEL /F /Q /A %konum%\Download\*
mkdir "%konum%\Download\Gerekli"
Files\wget --no-check-certificate %1 -O %down%\%~2 > NUL 2>&1
goto :eof
